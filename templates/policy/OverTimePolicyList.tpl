...
<div id="rowContentInner">
		...
		<form method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Active After{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="trigger_time" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{t}Functions{/t}
					</td>
				...
				{foreach from=$policies name=policies item=policy}
					...
					<tr class="{$row_class}">
						...
						<td>
							{gettimeunit value=$policy.trigger_time default=TRUE}
						</td>
						<td>
							{assign var="policy_id" value=$policy.id}
							{if $permission->Check('over_time_policy','edit')}
								[ <a href="{urlbuilder script="EditOverTimePolicy.php" values="id=$policy_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
