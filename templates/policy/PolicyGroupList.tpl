...
<div id="rowContentInner">
		...
		<form method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				{foreach from=$policies item=policy name=policy}
					...
					<tr class="{$row_class}">
						<td>
							{$smarty.foreach.policy.iteration}
						</td>
						<td>
							{$policy.name}
						</td>
						<td>
							...
							{if $permission->Check('policy_group','edit')}
								[ <a href="{urlbuilder script="EditPolicyGroup.php" values="id=$policy_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
