...
<div id="rowContentInner">
		...
		<form method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Next Date{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="next_date" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{t}Functions{/t}
					</td>
				...
				{foreach from=$rows item=row}
					...
					<tr class="{$row_class}">
						...
						<td>
							{getdate type="DATE" epoch=$row.next_date}
						</td>
						<td>
							{assign var="row_id" value=$row.id}
							{if $permission->Check('holiday_policy','edit')}
								[ <a href="{urlbuilder script="EditRecurringHoliday.php" values="id=$row_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
				...
				<tr>
					<td class="tblActionRow" colspan="7">
						{if $permission->Check('holiday_policy','add')}
							<input type="submit" class="button" name="action:add_presets" value="{t}Add Presets{/t}">
