				...
				{foreach from=$policies name=policies item=policy}
					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{if $permission->Check('exception_policy','edit')}
								[ <a href="{urlbuilder script="EditExceptionPolicyControl.php" values="id=$policy_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
