					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{if $permission->Check('holiday_policy','edit')}
								[ <a href="{urlbuilder script="EditHolidayPolicy.php" values="id=$policy_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
							{if $permission->Check('holiday_policy','edit')}
								[ <a href="{urlbuilder script="HolidayList.php" values="id=$policy_id" merge="FALSE"}">{t}Holidays{/t}</a> ]
							{/if}
						</td>
						<td>
