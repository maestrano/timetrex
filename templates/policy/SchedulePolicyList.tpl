...
<div id="rowContentInner">
		...
		<form method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Meal Policy{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="meal_policy_id" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{capture assign=label}{t}Absence Policy{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="absence_policy_id" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{capture assign=label}{t}Window{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="start_stop_window" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{t}Functions{/t}
					</td>
				...
				{foreach from=$policies name=policies item=policy}
					...
					<tr class="{$row_class}">
						...
						<td>
							{gettimeunit value=$policy.start_stop_window}
						</td>
						<td>
							{assign var="policy_id" value=$policy.id}
							{if $permission->Check('schedule_policy','edit')}
								[ <a href="{urlbuilder script="EditSchedulePolicy.php" values="id=$policy_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
