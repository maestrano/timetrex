...
<div id="rowContentInner">
		...
		<form method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Level{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="level" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{t}Functions{/t}
					</td>
					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{if $permission->Check('permission','edit') OR $permission->Check('permission','edit_own')}
								[ <a href="{urlbuilder script="EditPermissionControl.php" values="id=$row_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
