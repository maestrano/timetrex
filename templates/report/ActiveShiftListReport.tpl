							...
							<td>
								{if $key == 'function'}
									{if $permission->Check('punch','view') OR ( $permission->Check('punch','view_child') AND $user.is_child === TRUE ) OR ( $permission->Check('punch','view_own') AND $user.is_owner === TRUE ) }
										{assign var="user_id" value=$row.id}
										[ <a href="#" onClick="window.opener.location.href='{urlbuilder script="../timesheet/ViewUserTimeSheet.php" values="filter_data[user_id]=$user_id" merge="FALSE"}'">{t}View{/t}</a> ]
									{/if}
								{else}
									{$row[$key]|default:"--"}
								{/if}
