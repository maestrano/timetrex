								...
								<td valign="top" width="50%">
									<table class="editTable">
										...
										<tr id="tab_row_all">
											...
											<td class="cellRightEditTable">
												<select name="filter_data[punch_status_id]">
													{html_options options=$filter_data.punch_status_options selected=$filter_data.punch_status_id}
												</select>
											</td>
										</tr>
								...
								<td valign="top" width="50%">
									<table class="editTable">
										...
										<tr id="tab_row_all">
											...
											<td class="cellRightEditTable">
												<select name="filter_data[punch_type_id]">
													{html_options options=$filter_data.punch_type_options selected=$filter_data.punch_type_id}
												</select>
											</td>
										</tr>
										...
										<tr id="tab_row_adv_search">
											...
											<td class="cellRightEditTable">
												<select name="filter_data[punch_branch_id]">
													{html_options options=$filter_data.branch_options selected=$filter_data.punch_branch_id}
												</select>
											</td>
										</tr>
										<tr id="tab_row_adv_search">
											<td class="cellLeftEditTable">
												{t}Punch Department:{/t}
											</td>
											<td class="cellRightEditTable">
												<select name="filter_data[punch_department_id]">
													{html_options options=$filter_data.department_options selected=$filter_data.punch_department_id}
												</select>
											</td>
										</tr>
{*
				...
				{foreach from=$rows item=row name=row}
					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{assign var="date_stamp" value=$row.date_stamp}
							{if $permission->Check('punch','view') OR ( $permission->Check('punch','view_child') AND $row.is_child === TRUE ) OR ( $permission->Check('punch','view_own') AND $row.is_owner === TRUE ) }
							...
							{/if}
							{if $permission->Check('punch','edit') OR ( $permission->Check('punch','edit_child') AND $row.is_child === TRUE ) OR ( $permission->Check('punch','edit_own') AND $row.is_owner === TRUE ) }
								[ <a href="javascript:editPunch({$row_id})">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
