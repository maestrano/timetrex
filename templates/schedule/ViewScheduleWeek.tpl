...
<table class="tblList" id="schedule_table">
<form method="get" name="schedule" action="{$smarty.server.SCRIPT_NAME}">
		...
		<tr class="tblHeader" height="1">
		{foreach from=$calendar_array item=calendar name=calendar}
			<td id="cursor-hand" {if $permission->Check('schedule','edit') OR $permission->Check('schedule','edit_own') OR $permission->Check('schedule','edit_child')}onClick="TIMETREX.schedule.editSchedule('','',{$calendar.epoch})"{/if} nowrap>
