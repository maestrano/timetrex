...
{
        //This was causing sub-query with ORDER BY and LIMIT clauses to be stripped out resulting in multiple rows being returned and causing a SQL error.
	 ...
	 if (!empty($zthis->_nestedSQL) || preg_match("/^\s*SELECT\s+DISTINCT/is", $sql) || 
		...
		if ( strpos($sql, '_ADODB_COUNT') !== FALSE ) {
			$rewritesql = preg_replace(
				                '/^\s*?SELECT\s+_ADODB_COUNT(.*)_ADODB_COUNT\s/is','SELECT COUNT(*) ',$sql);
		} else {
			    $rewritesql = preg_replace(
				                '/^\s*?SELECT\s.*?\s+(.*?)\s+FROM\s/is','SELECT COUNT(*) FROM ',$sql);
		}
