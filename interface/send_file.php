	...
	case 'document':
		...
		if ( $rl->check() == FALSE ) {
			...
			if ( $permission->Check('document', 'view')
				...
				$result = $api_f->stripReturnHandler( $api_f->getDocument( array('filter_data' => array( 'id' => $parent_id ) ) ) );
				if ( isset($result[0]) AND count($result[0]) > 0 ) {
					$drlf = TTnew( 'DocumentRevisionListFactory' );
					$drlf->getByCompanyIdAndIdAndDocumentIdAndPrivateAllowed( $current_company->getId(), $object_id, $parent_id, $permission->Check('document', 'view_private') );
